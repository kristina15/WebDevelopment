...
public class UserLogic : BaseLogic, IUserLogic
{
    ...
    public UserLogic(IUserDao userDao, IPassportLogic passportLogic, IScanLogic scanLogic, IUserValidator userValidation, IScanValidator scanValidator)
    {
        _userDao = userDao;
    ...
    public void UpdateUser(User value)
    {
        ...
        else
        {
            foreach (var item in value.Passport.Scans)
            {
                item.Title = Entities.Enum.TitleTypesEnum.Passport;
