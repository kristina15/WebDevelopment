...
public class UserDao : BaseDao, IUserDao
{
                ...
                if (value == null)
                {
                    users.Add(new User
                    {
                        ...
                        AdditionalFile = reader["AdditionalScanId"] == DBNull.Value ? null : new ScanFile
                        {
                            ...
                            Title = (TitleTypesEnum)(int)reader["AdditionalScanTypeId"],
            ...
            while (reader.Read())
            {
                if (users.Where(u => u.Id == (int)reader["Id"]).Count() == 0)
                {
                    users.Add(new User
                    {
                        ...
                        FirstName = (string)reader["FirstName"],
                        Surname = (string)reader["Surname"],
                        Patronymic = reader["Patronic"] != DBNull.Value ? (string)reader["Patronic"] : null,
                        HashPassword = (byte[])reader["Password"],
                        Login = (string)reader["Login"],
                        Passport = new Passport
                        {
    ...
    private static User GetBasicUser(SqlDataReader reader)
    {
        return new User
        {
            ...
            Roles = reader["Title"] != DBNull.Value ? new List<string>
                    {
